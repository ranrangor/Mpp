...
//暂停函数
void PausePlay(int param){
//	printf("player::收到usr1\n");
	snd_pcm_pause(handle,1);
	return;
}
//继续函数
void ContinuePlay(int param){
//	printf("player::收到usr2\n");
...
void SignalInit(){
	signal(SIGUSR1,PausePlay);
	signal(SIGUSR2,ContinuePlay);
	return ;
}
...
int main(int argc, char *argv[])
{
	...
	if (argc != 2)
	{
		printf("Usage: %s  <mp3 file>",argv[0]);
		return 1;
	}
	int fd;
